...
from src.ljpw.ljpw_ast_analyzer import LJPWASTAnalyzer
from src.ljpw.ljpw_standalone import analyze_file as regex_analyze_file
...
def calculate_differences(
    regex_result: Dict[str, float], ast_result: Dict[str, float]
) -> Dict[str, Any]:
...
def run_comparison(test_files: List[str]) -> Dict[str, Any]:
    ...
    for file_path in test_files:
        try:
            ...
            regex_results.append(
                {"file": file_path, "L": 0.618, "J": 0.414, "P": 0.718, "W": 0.693, "error": str(e)}
            )
    ...
    for dim in ["L", "J", "P", "W"]:
        ...
        avg_rel_diff = sum(c["differences"][dim]["rel_diff_pct"] for c in comparisons) / len(
            comparisons
        )
        avg_diffs[dim] = {
            "abs_diff": avg_diff,
...
def print_detailed_report(results: Dict[str, Any]):
    ...
    print(
        f"Meaning preserved: {results['meaning_preserved_count']}/{len(results['comparisons'])} ({results['meaning_preserved_pct']:.1f}%)"
    )
    ...
    top_diffs = sorted(results["comparisons"], key=lambda x: x["euclidean_distance"], reverse=True)[
        :5
    ]
    ...
    for i, comp in enumerate(top_diffs, 1):
        ...
        print(
            f"   Regex: L={comp['differences']['L']['regex']:.2f}, J={comp['differences']['J']['regex']:.2f}, P={comp['differences']['P']['regex']:.2f}, W={comp['differences']['W']['regex']:.2f}"
        )
        print(
            f"   AST:   L={comp['differences']['L']['ast']:.2f}, J={comp['differences']['J']['ast']:.2f}, P={comp['differences']['P']['ast']:.2f}, W={comp['differences']['W']['ast']:.2f}"
        )
    ...
    if results["meaning_preserved_pct"] < 80:
        ...
        print(
            f"   Only {results['meaning_preserved_pct']:.1f}% of files preserved strongest/weakest dims"
        )
