...
""",
    ...
    def _build_code_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            'class_names': [
                'DataProcessor', 'CacheManager', 'AuthHandler', 'QueryBuilder',
                'EventEmitter', 'ConfigValidator', 'StateController', 'ServiceRegistry',
                'ConnectionPool', 'TaskScheduler', 'ResourceManager', 'ErrorHandler'
            ],
            'function_names': [
                'process_data', 'validate_input', 'compute_hash', 'fetch_resource',
                'parse_response', 'transform_format', 'filter_results', 'aggregate_data',
                'optimize_performance', 'synchronize_state', 'serialize_object', 'deserialize_data'
            ],
            'algorithm_names': [
                'binary_search', 'quick_sort', 'merge_sort', 'heap_sort',
                'depth_first_search', 'breadth_first_search', 'dijkstra', 'bellman_ford',
                'k_means_clustering', 'linear_regression', 'gradient_descent', 'newton_method'
            ],
            'variable_names': [
                'data', 'result', 'config', 'params', 'options', 'metadata',
                'response', 'request', 'headers', 'payload', 'session', 'context'
            ],
            'error_types': [
                'ValueError', 'TypeError', 'KeyError', 'IndexError',
                'ConnectionError', 'TimeoutError', 'AuthenticationError',
                'ValidationError', 'ConfigurationError', 'RuntimeError'
            ]
        }
    ...
    def generate_large_corpus(self, num_files: int, avg_lines_per_file: int) -> List[Tuple[str, str]]:
        ...
        for i in range(num_files):
            ...
            if current_lines < avg_lines_per_file:
                # Add more content
                additional_content = self._generate_additional_code(avg_lines_per_file - current_lines)
                code += '\n' + additional_content
...
def run_large_scale_corpus_test(corpus_sizes: List[int], avg_lines: List[int]):
    ...
    for result in results:
        if 'error' not in result:
            print(f"{result['files']:<8} {result['lines_per_file']:<8} {result['total_size_mb']:<10.1f} "
    ...
    if successful_results:
        ...
        print(f"Best Compression: {best_compression['files']} files "
              f"({best_compression['compression_ratio']:.1f}x ratio)")
        print(f"Fastest Throughput: {fastest_throughput['files']} files "
