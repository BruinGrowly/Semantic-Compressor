...
def analyze_token_usage():
    ...
    for case in test_cases:
        ...
        print(
            f"  Files per context window (original): {claude_limit / tokens_per_file_original:.0f}"
        )
        print(
            f"  Files per context window (LJPW): {claude_limit / (ljpw_tokens/case['files']):,.0f}"
        )
        print(
            f"  Analysis capacity increase: {(claude_limit / (ljpw_tokens/case['files'])) / (claude_limit / tokens_per_file_original):.0f}x"
        )
    ...
    for case in test_cases:
        ...
        print(
            f"{case['name']:<20} {original_tokens:<12,.0f} {ljpw_tokens:<8.1f} {savings:<12,.0f} {expansion:<12.0f}x"
        )
